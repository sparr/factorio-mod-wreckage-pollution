...
#
# WP_FACTORIO    the game binary, if it is not where Steam puts it here
# WP_FT_DATA     the throwaway data directory the run happens in
#
...
factorio="${WP_FACTORIO:-/home/sparr/Games/Steam/steamapps/common/Factorio/bin/x64/factorio}"
...
data="${WP_FT_DATA:-$HOME/.cache/wp-factorio-test}"
...
# wp-tests carries no prototypes; it is the gate that keeps the fixtures from registering
...
mkdir -p "$data/mods"
ln -sfn "$root/test/ft/wp-tests" "$data/mods/wp-tests"
